...
from solver import train_one_epoch, validate, eval_gres, eval_faith, eval_hal
...
def main():
    ...
    if args.no_eval:
        ...
        if args.eval_only:
            ...
            # logger.info(f"Training with {len(train_dataset)} examples and validating with {len(val_dataset)} examples, also validating on gRefCOCO with {len(grefcoco_val_ds)} examples, and validating on HalMetaBench with {len(test_dataset)}")
    ...
    if args.eval_only:
        if val_dataset.ds in ['grefcoco', 'refzom']:
            ...
            detail = {
                ...
                "imagename_list": imagename_list,
                "gt_list": gt_list,
                "pred_list": pred_list
            }
            with open(os.path.join(args.log_dir, 'hal_result_detail.json'), "w", encoding='utf-8') as f:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval:
            ...
            # _, _, hal_n_acc, hal_t_acc = eval_faith(test_loader, model_engine, epoch, args, logger, tokenizer)
            _, _, hal_n_acc, hal_t_acc = eval_faith(val_loader, model_engine, epoch, args, logger, tokenizer)
            # faith_giou, faith_ciou, faith_n_acc, faith_t_acc = eval_faith(val_loader, model_engine, epoch, args, logger, tokenizer)
