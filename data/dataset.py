                            ...
                            if self.ds == 'grefcoco_syn' or self.ds == 'grefcoco_hal' or self.ds == 'grefcoco_hal_pope':
                                ...
                                for rle_i in range(len(rle)):
                                    if not isinstance(rle[rle_i]["counts"], bytes):
                                        rle[rle_i]["counts"] = rle[rle_i]["counts"].encode()
                            else:
        ...
        if self.data_type == 'refer_seg':
            if self.ds == 'grefcoco_syn' or self.ds == 'grefcoco_hal' or self.ds == 'grefcoco_hal_pope':
                ...
                for idx_ref, strip_ref in enumerate(stripped_refers):
                    ...
                    # ans = f"{strip_ref}:[REJ]" if masks[idx_ref].sum() < 1.0 else f"{strip_ref}:[SEG]"
                    ans = f"Sure, the segmentation of {strip_ref} is [REJ]" if masks[idx_ref].sum() < 1.0 else f"Sure, the segmentation of {strip_ref} is [SEG]"
                    conv.append_message(
                        conv.roles[1],
                        ans + "."
                    )
