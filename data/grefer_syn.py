...
class G_REFER_SYN:
    def __init__(self, data_root, dataset="grefcoco_syn", splitBy="unc"):
        # provide data_root folder which contains grefcoco
        print("loading dataset %s into memory..." % dataset)
        self.ROOT_DIR = osp.abspath(osp.dirname(__file__))
        self.DATA_DIR = osp.join(data_root, dataset)
        if dataset in ["grefcoco_syn"]:
        ...
        if osp.exists(ref_file):
            ...
            # ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_v1.2_demo.json")
            ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_v1.2_extend.json")
            if osp.exists(ref_file):
                self.data["refs"] = json.load(open(ref_file, "rb"))
                # all_ref_data = json.load(open(ref_file, "rb"))
                # random.shuffle(all_ref_data)
                # self.data["refs"] = all_ref_data[:12000]
            else:
                raise FileNotFoundError("JSON file not found")

        # load annotations from data/dataset/instances.json
        # instances_file = osp.join(self.DATA_DIR, "instances_v1.2_demo.json")
        instances_file = osp.join(self.DATA_DIR, "instances_v1.2.json")
