...
class G_REFER_SYN2:
    def __init__(self, data_root, dataset="grefcoco_syn2", splitBy="unc"):
        # provide data_root folder which contains grefcoco
        print("loading dataset %s into memory..." % dataset)
        self.ROOT_DIR = osp.abspath(osp.dirname(__file__))
        self.DATA_DIR = osp.join(data_root, dataset)
        if dataset in ["grefcoco_syn2"]:
            self.IMAGE_DIR = "/home/ubuntu/researches/SA1B/SAM"
        else:
        ...
        if osp.exists(ref_file):
            ...
            # ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_syn2_test.json")
            ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_syn2_v4.1.json")
            # ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_HalMetaBench_train_v1.1.json")
            if osp.exists(ref_file):
        ...
        # instances_file = osp.join(self.DATA_DIR, "instances_syn2_test.json")
        instances_file = osp.join(self.DATA_DIR, "instances_syn2_v4.json")
        # instances_file = osp.join(self.DATA_DIR, "instances_HalMetaBench_train.json")
