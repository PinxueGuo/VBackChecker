...
class G_REFER_HAL:
    def __init__(self, data_root, dataset="grefcoco_hal", splitBy="unc"):
        # provide data_root folder which contains grefcoco
        print("loading dataset %s into memory..." % dataset)
        self.ROOT_DIR = osp.abspath(osp.dirname(__file__))
        self.DATA_DIR = osp.join(data_root, dataset)
        if dataset in ["grefcoco_hal"]:
        ...
        if osp.exists(ref_file):
            ...
            # ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_b1b.json")
            ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_HalMetaBench_checked_1.json")
            # ref_file = osp.join(self.DATA_DIR, f"grefs({splitBy})_POPE.json")
            if osp.exists(ref_file):
        ...
        # instances_file = osp.join(self.DATA_DIR, "instances_b1b.json")
        instances_file = osp.join(self.DATA_DIR, "instances_HalMetaBench_checkd_1.json")
        # instances_file = osp.join(self.DATA_DIR, "instances_POPE.json")
