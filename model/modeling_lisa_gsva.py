...
class LisaGSVAForCausalLM(LlavaLlamaForCausalLM):
    ...
    def model_forward(
        ...
        offset: torch.LongTensor,   # 每个batch的开始和结束的conversataion位置。bs=2, sample=5, vqa: [0,1,2], lisa: [0,5,10], grefcoco: [0,1,2]
        ...
        reeval: bool = False,
        query_ids: torch.LongTensor = None,
        query_attention_masks: torch.LongTensor = None,
        **kwargs,
    ):
        ...
        if inference: # Segmentation Eval
            ...
            for i in range(n_batch):
                ...
                if query_ids is not None:
                    gen_output_ids = self.generate(
                        images=images_clip_extend[: end_i - start_i],
                        input_ids=query_ids[start_i:end_i],
                        attention_mask=query_attention_masks[start_i:end_i],
                        max_new_tokens=128,
                        num_beams=1,
                    )
                    # gen_output_ids = gen_output_i.sequences
                    # pred_output_ids = gen_output_i.sequences[k]   # [SEG]: 32003, [REJ]: 32004, Because: 7311
                    # output_hidden_states.append(gen_output_i.hidden_states[0])
                else:
                    gen_output_ids = None
                ...
                if reeval:
                    # question prompt里的[REJ]不要动
                    # first_rej_true_msk = (torch.cumsum(input_ids==self.rej_token_idx, dim=1)==1) & (input_ids==self.rej_token_idx)
                    # Replace all [REJ] to [SEG], then re-eval
                    input_ids[input_ids == self.rej_token_idx] = self.seg_token_idx
                    # input_ids[first_rej_true_msk] = self.rej_token_idx
        ...
        # question prompt 里的seg token是不需要的，所以要去掉
        # first_seg_true_mask = (torch.cumsum(input_ids[:, 1:]==self.seg_token_idx, dim=1)==1) & (input_ids[:, 1:]==self.seg_token_idx)
        seg_token_mask = input_ids[:, 1:] == self.seg_token_idx # mask for gathering [SEG] tokens
        # seg_token_mask[first_seg_true_mask] = False
        ...
        # first_rej_true_mask = (torch.cumsum(input_ids[:, 1:]==self.rej_token_idx, dim=1)==1) & (input_ids[:, 1:] == self.rej_token_idx)
        rej_token_mask = input_ids[:, 1:] == self.rej_token_idx
        # rej_token_mask[first_rej_true_mask] = False
        ...
        for lang_i in range(len(input_ids)):
            ...
            mask_list_comp.append(this_seg_rej) # faith: [[0] 或 []]
        
        batch_is_nomask = []    # len=batch_size, when batch from VQA or FAITH = True
        for i_of in range(len(offset)-1):
            st, ed = offset[i_of], offset[i_of+1]
            this_batch_comp = mask_list_comp[st:ed]
            batch_is_nomask.append(any(len(comp) == 0 for comp in this_batch_comp))
        # batch_is_nomask[b]==True && do_segs[b]==True --> faith
        # batch_is_nomask[b]==True && do_segs[b]==False --> vqa
        ...
        pred_embeddings = pred_embeddings_  # 把pred_embeddings按conversation分组(list) 方便进SAM
        pred_masks = []
        # pred_ious = []
        ...
        if not inference: # training, only train the [SEG] masks, ignore [REJ] masks
            ...
            for k in range(len(offset) - 1):
                ...
                if len(select_comps) == 1:
                    ...
                    if batch_is_nomask[k]:
                        mask_list_comp_.append(
                            [i for i, comp in enumerate(select_comps) if len(comp) > 0]
                        )
                    else:
                        mask_list_comp_.append(select_comps)
            pred_masks = pred_masks_    # 再次按照batch分组, lisa那种从[1,1,1...]变成[5,5],而grefcoco没变化
            ...
            for b_idx in range(batch_size):
                ...
                if isinstance(mask_list_comp[b_idx], torch.Tensor):
                    ...
                    if batch_is_nomask[b_idx]:
                        this_pred_masks_[mask_list_comp[b_idx]] = pred_masks[b_idx]
                    else:
                        for j in range(L):
                            this_pred_masks_[j] = pred_masks[b_idx][j:j + 1][mask_list_comp[b_idx][j]]
                pred_masks_.append(this_pred_masks_)
            pred_masks = pred_masks_        # 在rej的那个位置，补全0mask。保证pred_masks和gt_masks的shape一致，
        
        if inference:
            if len(pred_masks) != len(gt_masks) and len(pred_masks) == gt_masks[0].shape[0]:
        ...
        for batch_idx in range(len(pred_masks)):
            ...
            if do_segs[batch_idx] == False:
                ...
                # continue  # 不行，会卡住
                # gt_mask = pred_mask.detach().clone()  # 不行，会卡住
